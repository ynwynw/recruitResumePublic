...
if(this.data.zhaopinfenlei){
    searchForm['zhaopinfenlei'] = '%' + this.data.zhaopinfenlei + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`zhaopinfenlei`, searchForm);
} else {
res = await list(`zhaopinfenlei`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zhaopinfenlei/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zhaopinfenlei",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/zhaopinfenlei/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/zhaopinfenlei/detail`
})
...
this.setData({
addAuth: utils.isAuth("zhaopinfenlei", "新增") || utils.isAuthFront("zhaopinfenlei", "新增"),
delAuth: utils.isAuth("zhaopinfenlei", "删除") || utils.isAuthFront("zhaopinfenlei", "删除"),
editAuth: utils.isAuth("zhaopinfenlei", "修改") || utils.isAuthFront("zhaopinfenlei", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("zhaopinfenlei",obj)
    }else{
         resList= await list("zhaopinfenlei",obj)
    }
