...
const {
data
} = await detail("zhaopinfenlei",id)
this.setData({
payAuth:utils.isAuthFront('zhaopinfenlei','支付')
})
...
const resUpdate = await update('zhaopinfenlei', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','zhaopinfenlei');
...
async reply() {
const detailList = this.data.detailList
const res = await update("zhaopinfenlei", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
